...
# This script trains our final model with more realistic data and provides a full evaluation report.
...
phishing_age = np.random.triangular(left=1, mode=30, right=1000, size=data[data['CLASS_LABEL'] == 1].shape[0]).astype(int)
# Legitimate sites: Mostly old, but with some new ones (e.g., new businesses)
legit_age = np.random.triangular(left=90, mode=3650, right=8000, size=data[data['CLASS_LABEL'] == 0].shape[0]).astype(int)
...
# Optional: A visual plot of the confusion matrix
try:
    ...
    print("NOTE: Seaborn and Matplotlib not found. Skipping plot. To install: pip install seaborn matplotlib")
