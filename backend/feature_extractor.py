...
def get_num_dashes(url):
    return url.count("-")


def get_num_dashes_in_hostname(url):
...
def get_num_query_components(url):
    ...
    return len(
        [
            item
            for sublist in [q.split("=") for q in query.split("&")]
            for item in sublist
        ]
    )
...
def is_http_scheme(url):
    return 1 if urlparse(url).scheme == "http" else -1
...
# In backend/feature_extractor.py

# In backend/feature_extractor.py

# In backend/feature_extractor.py - FINAL version

def has_domain_in_subdomains(url):
    """
    Checks if the subdomain contains a different registered domain, a key phishing tactic,
    while ignoring common subdomains like 'www'.
    """
    ext = tldextract.extract(url)
    
    # If there is no subdomain, it can't be a deceptive one.
    if not ext.subdomain:
        return -1
    
    # --- THIS IS THE NEW LOGIC ---
    # Ignore common, non-deceptive subdomains.
    if ext.subdomain.lower() in ['www', 'ww2', 'm', 'mobile']:
        return -1
    # ---------------------------
        
    # Check if the subdomain itself could be a registered domain.
    # e.g., in 'google.com.malicious.com', the subdomain is 'google.com'.
    subdomain_ext = tldextract.extract(f"http://{ext.subdomain}")
    
    # If the subdomain has a valid domain part ('google') and it's different
    # from the main URL's domain part ('malicious'), it's a deceptive URL.
    if subdomain_ext.domain and subdomain_ext.domain != ext.domain:
        return 1
...
def get_num_sensitive_words(url):
    ...
    return sum(
        len(re.findall(r"\b" + word + r"\b", url.lower())) for word in sensitive_words
    )
...
def extract_features_from_url(url):
    ...
    features = {
        ...
        "NumDash": get_num_dashes(url),
        "NumDashInHostname": get_num_dashes_in_hostname(url),
        "AtSymbol": has_at_symbol(url),
        "TildeSymbol": has_tilde_symbol(url),
        "NumUnderscore": get_num_underscores(url),
        "NumPercent": get_num_percents(url),
        "NumQueryComponents": get_num_query_components(url),
        "NumAmpersand": get_num_ampersands(url),
        "NumHash": get_num_hash(url),
        "NumNumericChars": get_num_numeric_chars(url),
        "NoHttps": is_http_scheme(url),
        "RandomString": has_random_string(url),
        "IpAddress": has_ip_address(url),
        "DomainInSubdomains": has_domain_in_subdomains(url),
        "DomainInPaths": has_domain_in_paths(url),
        "HttpsInHostname": has_https_in_hostname(url),
        "HostnameLength": get_hostname_length(url),
        "PathLength": get_path_length(url),
        "QueryLength": get_query_length(url),
        "DoubleSlashInPath": has_double_slash_in_path(url),
